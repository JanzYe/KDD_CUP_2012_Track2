...
num_train = NUM_TRAINING * (10.0 / 11)
# num_train = 360000
num_valid = NUM_TRAINING * (1.0 / 11)

# chunk_size = 10000000
chunk_size = 1800000
chunk_size_test = 1800000

# 隐藏层单元数不是越高越好，中间有一个临界值达到最优.
# Dropout在数据量本来就很稀疏的情况下尽量不用，不同的数据集dropout表现差距比较大
batch_size = 18000
embedding_size = 20  # embedding cost most of the memory, if OOM, reduce this
...
def processing_df(df, shuffling=True):
    ...
    return df, multi_vals, valid_lens, dense_need_combine_val

def processing_batch(df, multi_vals, valid_lens, dense_need_combine_val, cnt, batch_size, len_df, label=True):
    end = cnt+batch_size
    if end > len_df:
        end = len_df
        cnt = end - batch_size
    ...
    # print(multi_vals)
    if label:
        Y =[batch[key].values for key in target]
    else:
        Y = []
...
def generate_train_from_file(path_combined_mapped, batch_size):
    while 1:
        reader = pd.read_csv(path_combined_mapped, chunksize=chunk_size)

        print('epoch start')
        for df in reader:
            df, multi_vals, valid_lens, dense_need_combine_val = processing_df(df)
            cnt = 0
            len_df=df.shape[0]
            while cnt < len_df:
                X, Y = processing_batch(df, multi_vals, valid_lens, dense_need_combine_val, cnt, batch_size, len_df)
                yield (X, Y[0]/Y[1])
                cnt += batch_size
            ...
            del multi_vals
            del valid_lens
            del dense_need_combine_val
            gc.collect()
        
        print('epoch complete')
        
def generate_valid_from_file(path_combined_mapped):
    
    print('generating data')
    df = pd.read_csv(path_combined_mapped)
    ...
    X, Y = processing_batch(df, multi_vals, valid_lens, dense_need_combine_val, cnt, len_df, len_df)
...
def generate_test_from_file(path_combined_mapped):
    
    print('generating data')
    df = pd.read_csv(path_combined_mapped)

    print('processing_df')
    df, multi_vals, valid_lens, dense_need_combine_val = processing_df(df, False)
    cnt = 0
    len_df=df.shape[0]
    print('processing_batch')
    X, _ = processing_batch(df, multi_vals, valid_lens, dense_need_combine_val, cnt, len_df, len_df, False)
...
if __name__ == "__main__":
    ...
    dense_features = [
        ...
        'DisplayURL', 'AdID', 'AdvertiserID', 'QueryID', 'KeywordID', 'TitleID', 'DescriptionID', 'UserID',
    ...
    # if (len(multi_val_features_dim)) > 0 or (len(dense_need_combine_features) > 0):
    ...
    if mode == TRAIN:
        ...
        x_valid, y_valid = generate_valid_from_file(PATH_VALID)
        
        checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min',
                                     save_weights_only=True)
        
        auc_eval = AucCallback(y_valid[0], y_valid[1], x_valid, batch_size)
        print(np.sum(y_valid[0]/y_valid[1]))

        history = model.fit_generator(generate_train_from_file(PATH_TRAIN, batch_size=batch_size),
                                      steps_per_epoch=int(np.ceil(num_train/batch_size)), 
                                      callbacks=[checkpoint, auc_eval], epochs=20, verbose=1,
        ...
        reader = pd.read_csv(PATH_VALID, chunksize=chunk_size)
        ...
        for df in reader:
            ...
            while cnt < len_df:
                ...
                labels.extend(batch[target].values)
...
#         model_name = 'deep_fm_combined-ep010-loss0.127-val_loss0.127-bs18000-ee8-hz[512, 256, 256, 256, 128]-l2l1e-06-l2e1e-06-l2d0.001-t2019-01-03 03:45:50.h5'  # auc: 0.774472 0.784256
        ...
        target = []
        print('generating ......')
        x_test = generate_test_from_file(PATH_TEST)
        preds = model.predict(x_test, batch_size, verbose=1)
